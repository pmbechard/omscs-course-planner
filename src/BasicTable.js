...
function BasicTable({
  ...
  const handleHeaderClick = (event) => {
    ...
    if (
      clickedHeader === currentlySortedBy ||
      ['↑', '↓'].includes(clickedHeader)
    ) {
      setSortDirection(
  ...
  const sortRows = (sortHeader) => {
    let propertyName;
    if (sortHeader === 'Course') {
      propertyName = 'name';
    } else if (sortHeader === 'Foundational?') {
      propertyName = 'isFoundational';
    } else if (sortHeader === 'Rating') {
      propertyName = 'rating';
    } else if (sortHeader === 'Difficulty') {
      propertyName = 'difficulty';
    } else if (sortHeader === 'Rating:Difficulty') {
      propertyName = 'ratingDifficultyRatio';
    } else if (sortHeader === 'Workload') {
      propertyName = 'workload';
    } else if (sortHeader === 'Rating:Workload') {
      propertyName = 'ratingWorkloadRatio';
    } else if (sortHeader === 'Reviews') {
      propertyName = 'reviewCount';
    } else if (sortHeader === 'Code(s)') {
      propertyName = 'codes';
    }
    setSortedRows(
      rows.toSorted((a, b) =>
        sortCompareFunction(a, b, propertyName, sortDirection)
      )
    );
          ...
          <table className='table table-bordered' id={tableId}>
            <thead>
              <tr>
                ...
                <th scope='col' onClick={handleHeaderClick}>
                  Foundational?{' '}
                  {currentlySortedBy === 'Foundational?' ? (
                    sortDirection == 'ascending' ? (
                      <span className='arrow'>↑</span>
                    ) : (
                      <span className='arrow'>↓</span>
                ...
                <th scope='col' onClick={handleHeaderClick}>
                  Rating:Difficulty{' '}
                  {currentlySortedBy === 'Rating:Difficulty' ? (
                    sortDirection == 'ascending' ? (
                      <span className='arrow'>↑</span>
                    ) : (
                      <span className='arrow'>↓</span>
                ...
                <th scope='col' onClick={handleHeaderClick}>
                  Rating:Workload{' '}
                  {currentlySortedBy === 'Rating:Workload' ? (
                    sortDirection == 'ascending' ? (
                      <span className='arrow'>↑</span>
                    ) : (
                      <span className='arrow'>↓</span>
            ...
            <tbody>
              {sortedRows.length === 0 ? (
                <tr className='empty-table'>
                  <td>
                    There's nothing here yet. Pick a specialization and add some
                    classes to populate your course list!
                  </td>
                </tr>
              ) : (
                sortedRows.map(
                    ...
                    >
                      ...
                      <td>
                        ...
                        <div>
                          ...
                          <a
                            href={
                              'https://www.omscentral.com/courses/' +
                              slug +
                              '/reviews'
                            }
                            target='_blank'
              ...
              {tableId === 'chosenCourses' && sortedRows.length !== 0 && (
                <tr className='averages-row'>
                  <td>
                    <div>Average Value</div>
                  </td>
                  <td>{getAvg('rating')}</td>
                  <td>{getAvg('difficulty')}</td>
                  <td>{getAvg('workload')}</td>
                  <td>{getAvg('reviewCount')}</td>
                  <td></td>
                  <td>{getAvg('rating', 'difficulty')}</td>
                  <td>{getAvg('rating', 'workload')}</td>
                  <td></td>
                </tr>
              )}
            </tbody>
